...
- `/openapi.json` と `/docs/api` はログインなしで公開。管理用APIと管理用スキーマは公開仕様から除外し、それ以外のルートを文書化してCLI非対応の操作を区別する。生成元 `scripts/openapi.py`、登録ルートと仕様の対応をテスト。既存バリデーションを維持し、大規模なルーター置換は行わない。
