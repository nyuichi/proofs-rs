...
add(P+'/runs/{run}/sarif','post','Upload immutable SARIF with embedded logs',out=obj({'sha256':S(),'size':I}),status=201,description='Upload SARIF 2.1.0 (max 8 MiB including embedded stdout/stderr) as the raw request body. Source files are not accepted. Exact retries succeed; changing an artifact is rejected. 90 new artifacts per user per UTC day.')
paths[P+'/runs/{run}/sarif']['post']['requestBody']={'required':True,'content':{'application/octet-stream':{'schema':{'type':'string','format':'binary'}}}}
add(P+'/runs/{run}','post','Finalize an immutable recorded run',body={'type':'object','description':'Run metadata: crate, version, tool_version_id, command (argv), working_directory, started_at, finished_at, duration_ms, exit_code, execution_successful, contracts, sarif_sha256, source (repository HTTPS GitHub URL and full commit SHA), platform, rustc and environment. SARIF must already exist. Contracts must map to successful SARIF results.'},out=obj({'id':S(format='uuid')}),status=201)
