...
add(P+'/runs/{run}/sarif','post','Register an immutable verification run with one SARIF upload',out=obj({'id':S(format='uuid'),'sha256':S()}),status=201,description='Upload SARIF 2.1.0 (max 8 MiB including embedded stdout/stderr) as the raw request body. See the SARIF profile in docs/sarif.md. The service validates the complete record before storage; no separate finalization request exists. Source files are not accepted. Exact retries succeed; changing an artifact is rejected. 90 new runs per user per UTC day.')
paths[P+'/runs/{run}/sarif']['post']['requestBody']={'required':True,'content':{'application/octet-stream':{'schema':{'type':'string','format':'binary'}}}}
...
add(P+'/runs/{run}/sarif','get','Download SARIF with embedded execution logs',out={'type':'string','format':'binary'},description='Returns a SARIF attachment; access follows report visibility.')
paths[P+'/runs/{run}/sarif']['get']['responses']['200']['content']={'application/octet-stream':{'schema':{'type':'string','format':'binary'}}}
...
add(P+'/crates/{name}/{version}/apis','get','List imported public functions and concrete methods',out=listing({'allOf':[ref('api_items'),obj({'no_ub_count':{'type':'integer'},'panic_count':{'type':'integer'}})]}),queries=cursor+[('q',S(),False)])
